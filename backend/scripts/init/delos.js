    ...
    return __awaiter(this, void 0, void 0, function* () {
        for (let BO_MODULE_INDEX = 0; BO_MODULE_INDEX < bo_3.BO_MODULE_COUNT; BO_MODULE_INDEX++) {
            ...
            for (let BO_INTERFACE_INDEX = 0; BO_INTERFACE_INDEX < bo_3.BO_INTERFACE_COUNT; BO_INTERFACE_INDEX++) {
                let itf = yield index_1.Interface.create(bo_2.BO_INTERFACE_FN({ creatorId: mod.creatorId, repositoryId: repository.id, moduleId: mod.id }));
                yield mod.$add('interfaces', itf);
                for (let BO_PROPERTY_INDEX = 0; BO_PROPERTY_INDEX < bo_3.BO_PROPERTY_COUNT; BO_PROPERTY_INDEX++) {
                    let prop = yield index_1.Property.create(bo_2.BO_PROPERTY_FN({ creatorId: itf.creatorId, repositoryId: repository.id, moduleId: mod.id, interfaceId: itf.id }));
